...
const sellfun = async (client) => {
    ...
    if (GetOpenOrders.length > 0) {
        let CancelOpenOrders = await client.CancelOpenOrders({ symbol: SYMBOL });
        console.log(getNowFormatDate(), "All pending orders canceled");
    }

    let userbalance2 = await client.Balance();
    console.log(getNowFormatDate(), `My Account Infos: ${userbalance2.PYTH.available} $PYTH | ${userbalance2.USDC.available} $USDC`,);

    let { lastPrice: lastPriceask } = await client.Ticker({ symbol: SYMBOL });
    console.log(getNowFormatDate(), "Price pyth_usdc:", lastPriceask);
    let quantitys = (userbalance2.PYTH.available - 0.2).toFixed(1);
    console.log(getNowFormatDate(), `Trade... ${quantitys} $PYTH to ${(lastPriceask * quantitys).toFixed(1)} $USDC`);
...
const buyfun = async (client) => {
    ...
    let balanceSol = 0;
    if (userbalance.PYTH) {
        balanceSol = userbalance.PYTH.available
    }
    console.log(getNowFormatDate(), `My Account Infos: ${balanceSol} $PYTH | ${userbalance.USDC.available} $USDC`,);
    let { lastPrice } = await client.Ticker({ symbol: SYMBOL });
    console.log(getNowFormatDate(), "Price of pyth_usdc:", lastPrice);
    let quantitys = ((userbalance.USDC.available - 2) / lastPrice).toFixed(1);
    console.log(getNowFormatDate(), `Trade ... ${(userbalance.USDC.available - 2).toFixed(1)} $USDC to ${quantitys} $PYTH`);
